...
from oneHourFuturePointFile.futures import futures


def get_content(symbol, proxy, tries=0):
    try:
        url = f"https://finance.yahoo.com/quote/{symbol}=F?p={symbol}=F"
        if proxy:
...
def lambda_handler(event, context):
    ...
    name = get_date_name()
    s3.upload_file("/tmp/temp_data.json", "future-listings-latest", f"{name}.json")
