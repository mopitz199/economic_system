...
from oneHourCurrencyPointFile.currencies import currencies


def get_content(symbol, proxy, tries=0):
    try:
        url = f"https://finance.yahoo.com/quote/{symbol}=X?p={symbol}=X"
        if proxy:
...
def get_date_name():
    ...
    return date_obj.strftime("%Y%m%d%H0000")
...
def lambda_handler(event, context):
    ...
    name = get_date_name()
    s3.upload_file("/tmp/temp_data.json", "currency-listings-latest", f"{name}.json")
